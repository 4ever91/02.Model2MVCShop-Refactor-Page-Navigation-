...
public class Page {
	...
	public Page( int currentPage, int totalCount,	int pageUnit, int pageSize ) {
		...
		//최대페이지 번호 = 한페이지당 보여지는 게시물수가 0 ? 총게시물수 : 총게시물수 -1 /페이지당 보여지는 게시물수 +1
		this.maxPage = (pageSize == 0) ? totalCount :  (totalCount-1)/pageSize +1;
		//현재 페이지 =현재페이지>최대 페이지 번호 ? 최대페이지 : 현재 페이지 
		this.currentPage = ( currentPage > maxPage) ? maxPage : currentPage;
		//화면에 보여지는 페이지 번호의 최소수 = ((현재페이지 -1)/하단페이지 번호화면에 보여지는 수) *하단페이지 번호화면에 보여지는수 +1 
		this.beginUnitPage = ( (currentPage-1) / pageUnit ) * pageUnit +1 ;
		//최대페이지 <= 하단에 보여지수 
		if( maxPage <= pageUnit ){
			//화면에 보여지는 보여지는 최대수 =최대 페이지 번호
			this.endUnitPage = maxPage;
		}else{
			/// 보여지는 최대수 = 최소수 + 하단 보여지는 수 -1
